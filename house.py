...
def get_monthly_payment(loanAmt, annualInterestPercent, numPayments):
    ...
    for i in range(0, int(numPayments)):
...
print("$%s: Monthly mortgage payment" % locale.format_string("%d", monthly[0].payment, grouping=True))
print("$%s: Total monthly expenses" % locale.format_string("%d", total_monthly_expense, grouping=True))
print("$%s: Monthly income" % locale.format_string("%d", params.rent, grouping=True))
print("$%s: Monthly cashflow" % locale.format_string("%d", params.rent-total_monthly_expense, grouping=True))
...
print("$%s: Interest in %d years" % (locale.format_string("%d", cumulativeInterest, grouping=True), params.duration))
print("$%s: Cost of invesement in %d years " % (locale.format_string("%d", total_investment_cost, grouping=True), params.duration))
print("$%s: Total income in %d years" % (locale.format_string("%d", params.rent * durationMonths, grouping=True), params.duration))
print("$%s: Total income with appreciation" % locale.format_string("%d",total_income, grouping=True))
print("$%s: Sale price" % locale.format_string("%d", sale_price, grouping=True))
print("$%s: Remaining loan at time of sale" % locale.format_string("%d", remaining_loan_amount, grouping=True))
print("$%s: Cash in hand at sale (after selling expenses)" % locale.format_string("%d", cash_out, grouping=True))
print("$%s: Net gain/loss"  % locale.format_string("%d", total_income - total_investment_cost, grouping=True))
